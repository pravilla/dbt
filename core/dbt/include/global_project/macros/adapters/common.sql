...
{% macro _dbt_drop_relation(relation) -%}
...
{% macro _dbt_truncate_relation(relation) -%}
...
{% macro _dbt_rename_relation(from_relation, to_relation) -%}
...
{% macro _dbt_list_schemas(database) -%}
...
{% macro default__list_schemas(database) -%}
  {% call statement('list_schemas', fetch_result=True, auto_begin=False) %}
    ...
    from {{ dbt.information_schema_name(database) }}.schemata
...
{% macro _dbt_check_schema_exists(database, schema) -%}
